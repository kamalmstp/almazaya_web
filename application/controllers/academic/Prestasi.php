...
class Prestasi extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'Prestasi Siswa';
		$this->vars['academic'] = $this->vars['academic_references'] = $this->vars['prestasi'] = TRUE;
		$this->vars['content'] = 'academic/prestasi';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_prestasi->get_where($keyword, 'rows', $limit, $offset);
			$total_rows = $this->m_prestasi->get_where($keyword);
	...
	private function validation() {
		...
		$val->set_rules('title', 'Nama Siswa', 'trim');
		$val->set_rules('content', 'Konten', 'trim');
	...
	public function upload() {
		if ($this->input->is_ajax_request()) {
			...
			if (_isNaturalNumber( $id )) {
				...
				$config = [];
				$config['upload_path'] = './media_library/prestasi/';
				...
				if ( ! $this->upload->do_upload('file')) {
					...
					if ($update) {
						// chmood new file
						@chmod(FCPATH.'media_library/prestasi/'.$file['file_name'], 0777);
						// chmood old file
						@chmod(FCPATH.'media_library/prestasi/'.$file_name, 0777);
						// unlink old file
						@unlink(FCPATH.'media_library/prestasi/'.$file_name);
						// resize new image
						$this->image_resize(FCPATH.'media_library/prestasi', $file['file_name']);
					}
	...
	/**
	 ...
	 private function image_resize($path, $file_name) {
		...
		$config['width'] = (int) __session('prestasi_width');
		$config['height'] = (int) __session('prestasi_height');
