...
class Employee_profile extends Admin_Controller {
			...
			if (_isNaturalNumber( $id )) {
				if ($this->validation( $id )) {
					...
					if ($this->vars['status'] == 'success') {
						...
						$email = $dataset['email'];
						if ($email != __session('user_email')) {
							$this->load->model('m_users');
							$query = $this->m_users->reset_user_email($email);
							if ($query) $this->session->set_userdata('user_email', $email);
						}
					}
				} else {
	...
	public function email_exists( $email, $id ) {
		...
		foreach($emails as $data) {
			if ( ($data['id'] != $id && $data['email'] == $email && $data['user_type'] == 'is_employee') OR
				($data['email'] == $email && $data['user_type'] == 'is_student') OR
