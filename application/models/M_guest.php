...
class M_guest extends CI_Model {
	 ...
	 * @return Boolean
	 */
	public function insert($selection_result, array $guest_number = []) {
		$approved = 0;
		$unapproved = 0;
		$error = 0;
		foreach($guest_number as $ge_number) {
			if ($selection_result != 'unapproved') {
				if ($this->check_quota($ge_number)) {
					$query = $this->db
						->where('guest_number', $ge_number)
	 ...
	 * @return Boolean
	 */
	private function check_quota($guest_number) {
		// Get First Choice
		...
		$approved = $this->db
			...
			->where('LEFT(guest_number, 4) = ', $this->admission_year)
	 ...
	 * @return Boolean
	 */
	public function guest_number() {
		$admission_year = $this->admission_year;
		$query = $this->db->query("
			SELECT MAX(RIGHT(guest_number, 6)) AS max_number
			FROM students
			WHERE is_prospective_student='true'
			AND LEFT(guest_number, 4) = ?
		", [$admission_year]);
		$guest_number = "000001";
		if ($query->num_rows() === 1) {
			$data = $query->row();
			$number = ((int) $data->max_number) + 1;
			$guest_number = sprintf("%06s", $number);
		}
		return $admission_year . $guest_number;
	}
	 ...
	 * @return Array
	 */
	public function selection_result($guest_number) {
		$query = $this->db
			->where('guest_number', $guest_number)
			->get(self::$table);
		if ($query->num_rows() === 1) {
	 ...
	 * @return Boolean
	 */
	public function is_valid_guest($id) {
		$this->db->where('id', $id);
		$count = $this->db->count_all_results(self::$table);
		return $count > 0;
	}

	public function guest_query() {
		return "
			SELECT x1.id
				, x1.created_at
